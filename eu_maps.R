...
data_merged=merge(data_merged, country_name_abb, by="Country", all.x =T)
...
## Tables building
table_top3_hea_lif <<- gvisTable(top3_hea_lif[,c("Name","Healthy_Life" )])
table_top3_lif_exp <<- gvisTable(top3_lif_exp[,c("Name","Life_Expectancy" )])
...
## Tables building
table_top3_gap_hi <<- gvisTable(top3_highest_gap[,c("Name","age_gap" )])
